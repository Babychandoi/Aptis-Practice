...
import { assetApi, mockTestApi, uploadToPresignedUrl } from '@/api/endpoints';
...
function PostEditor({
  ...
  const [linkedBlueprints, setLinkedBlueprints] = useState<{ id: string; name: string }[]>([]);
...
${snippet}` }));
  ...
  useEffect(() => {
    ...
    setLinkedBlueprints(
      (post.blueprints ?? []).map((item) => ({ id: item.blueprintId, name: item.name })),
    );
    setForm({
      title: post.title,
  ...
  const saveMutation = useMutation({
    mutationFn: (status: NewsPostStatus) => {
      const payload: SaveNewsPostRequest = {
        ...
        blueprintIds: linkedBlueprints.map((item) => item.id),
      ...
      >
        ...
        {postId && existingQuery.isPending ? (
          ...
          <div className="flex-1 space-y-4 p-5">
            ...
            <BlueprintPicker selected={linkedBlueprints} onChange={setLinkedBlueprints} />
...
/**
 * Chọn đề thi thử đủ 4 phần gắn vào bài.
 *
 * Khác QuestionSetPicker (chọn bộ đề LẺ, học viên bấm vào chỉ làm một phần):
 * bài hướng dẫn cả kỹ năng thì cần mở một lượt đi hết Part 1 đến Part 4.
 */
function BlueprintPicker({
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setQuery(term.trim().toLowerCase()), 300);
    return () => window.clearTimeout(timer);
  }, [term]);

  // API đề thi thử không nhận từ khoá tìm kiếm, nên tải danh sách rồi lọc tại
  // chỗ. Số đề thi thử chỉ vài chục nên không đáng phải thêm endpoint mới.
  const listQuery = useQuery({
    queryKey: ["admin", "news", "blueprints"],
    queryFn: () => mockTestApi.list(undefined, 0, 200),
  });

  const matches = (listQuery.data?.content ?? []).filter((item) =>
    query.length === 0 ? false : item.name.toLowerCase().includes(query),
  );
  ...
  return (
    <div className="rounded-xl border border-brand-300 bg-brand-50/60 p-4">
      ...
      <input
        ...
        placeholder="Tìm đề thi thử, ví dụ: Writing Music Club"
        aria-label="Tìm đề thi thử"
      />
          ...
          {listQuery.isPending ? (
            ...
            <ul className="divide-y divide-slate-100">
              {matches.slice(0, 30).map((item) => {
                const already = selected.some((x) => x.id === item.id);
                return (
                  <li key={item.id}>
                    <button
                      type="button"
