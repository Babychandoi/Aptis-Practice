// Sinh scripts/seed-speaking-part2-batch3.sql từ mảng TOPICS trong
// scripts/seed-speaking-part2-batch3.js, để metadata MySQL và nội dung Mongo luôn khớp.
//
// Chạy:  node scripts/gen-speaking-part2-batch3-sql.js

const fs = require('fs');
const path = require('path');

const dir = __dirname;
const src = fs.readFileSync(path.join(dir, 'seed-speaking-part2-batch3.js'), 'utf8');
...
const header = `-- Metadata MySQL cho 15 đề Speaking Part 2 (LÔ 3).
--
...
const rows = TOPICS.map((entry, index) => {
  ...
  return `('${id}', @part_id, @task_type_id, @topic_id, 'SPEAKING_P2_C${suffix}', '${esc(title)}', 3, ${entry.hot}, ${year}, '${access}', 'PUBLISHED', 1, 3, 180, 15.00, NOW(), NOW(), NOW())`;
});
...
fs.writeFileSync(
  path.join(dir, 'seed-speaking-part2-batch3.sql'),
  header + rows.join(',\n') + footer,
  'utf8');
