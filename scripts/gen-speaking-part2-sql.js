// Sinh scripts/seed-speaking-part2.sql từ mảng TOPICS trong
// scripts/seed-speaking-part2.js, để metadata MySQL và nội dung Mongo luôn khớp.
//
// Chạy:  node scripts/gen-speaking-part2-sql.js

const fs = require('fs');
const path = require('path');

const dir = __dirname;
const src = fs.readFileSync(path.join(dir, 'seed-speaking-part2.js'), 'utf8');
...
const header = `-- Metadata MySQL cho 17 đề Speaking Part 2.
--
...
const rows = TOPICS.map((entry, index) => {
  ...
  const id = 'a5000000-0000-4000-8000-' + String(index + 1).padStart(12, '0');
  ...
  return `('${id}', @part_id, @task_type_id, @topic_id, 'SPEAKING_P2_${suffix}', '${esc(title)}', 3, ${entry.hot}, ${year}, '${access}', 'PUBLISHED', 1, 3, 180, 15.00, NOW(), NOW(), NOW())`;
});
...
fs.writeFileSync(
  path.join(dir, 'seed-speaking-part2.sql'),
  header + rows.join(',\n') + footer,
  'utf8');
